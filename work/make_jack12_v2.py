...
SCROLL_COPY = {
    ...
    "passion": {
        ...
        "why_intro": "値段で選ぶ場ではありません。誰と出会えるか、どんな仲間とつながれるか。JACK12が世界の第一線で積んだ実体験と、志を持った経営者が、ここに集まっています。",
        "why_bullets": [
            ("世界の第一線で積んだ、生きた現場の知恵", "王族や要人を前にした式典、映画祭の舞台。そこで実際に起きたことを、余さず伝えます。"),
            ("肩書きを、信頼と求心力に", "実績を信頼につなげ、また会いたいと思われる話し方を身につけます。"),
...
PROGRAMS = [
    ...
    ("JACK12オンライン塾", "550,000円（税込）", "とにかく学びたい方へ。グループオンライン講座で、ユダヤ人に伝わる原理原則、人から信頼される考え方、自分の見せ方を順を追って学びます。"),
    ("JACK12 VIP塾", "3,300,000円（税込）", "一緒にビジネスをしたい方へ。マンツーマンで、原理原則から事業の組み立てまで。海外の王族・要人や日本企業の経営者と出会う機会の生かし方も、一緒に掘り下げます。"),
]
...
MEDIA_BULLETS = [
    ...
    ("大阪・関西万博 EXPO 2025 出演", "リッツ・カールトン京都、リーガロイヤルホテル京都ほか一流ホテル・式典での公演実績。"),
    ("競技マジックで日本代表", "アジア国際大会（深圳）日本代表ファイナリスト。日本マジック教育普及協会 理事・師範。"),
]

TRUST01 = [
    ("バリ島名誉親善大使", "バリ島王様公認。国際親善・文化交流の活動背景として紹介。"),
...
TRUST02 = [
    ...
    ("王族・国家元首に認められた活動背景", "王族、貴族、国家元首、大統領、国際機関より招請を受けた公式プロフィール上の国際活動。"),
...
DISCLAIMER = "掲載内容はご提供プロフィール・公開用素材に基づく紹介文です。人脈形成、商談成立、成果を保証するものではありません。価格はすべて税込表記です。"
...
def build_scroll(tone):
    ...
    y = image_card(y + 8, "IMG_8280 2.JPG", "上質な場での対話や交流を意識したプログラムです", (0.43, 0.48), 430)
    ...
    y += 16
    y = section(y, "MEDIA & STAGE", "万博の舞台と、受賞歴", "華やかな国際活動を支えているのは、メディア出演や受賞、公演といった、誰でも確かめられる実績です。")
    ...
    # --- PROGRAM cards ---
    y += 26
    y = section(y, "PROGRAM", "ご縁の結び方は、3通り", "まず雰囲気を知る道、じっくり学ぶ道、二人三脚で取り組む道。\n今の目的に合わせて選べます。")
    ...
    y += 26
    y = ml(mm, y, "公式プロフィールによると、王様公認で「名誉親善大使」の称号を授かったエンターテイナーです。王族や国家元首、国際機関などとの接点を生かし、エンターテインメントを通じて国際親善や次世代教育、平和構築に取り組んでいます。", mf["body"], COLORS["muted"], mw - 2 * mm, 14)

    # --- TRUST 01 ---
    y += 42
    y = section(y, "TRUST 01", "親善大使としての任命と受賞", "バリ島王様公認の名誉親善大使をはじめ、各国の親善大使や国際プログラムへの招聘、出演、実行委員、受賞歴。こうした積み重ねが信頼の支えになっています。")
    ...
    # --- TRUST 02 ---
    y = section(y, "TRUST 02", "王族・要人の前に立った舞台", "王族関係者や各国大臣、国際式典、カンヌ、モナコなどの場で出演・披露してきました。公式プロフィールでも、こうした国際舞台での評価が大きな強みとされています。")
...
FLYER_COPY = {
    ...
    "passion": {
        ...
        "cards_sub": "まず覗いてみる、本気で学ぶ、二人三脚で挑む。あなたに合う入り方を選べます。",
        "foot_sub": "まず覗く / 本気で学ぶ / 二人三脚で挑む",
    },
}

FLYER_CARDS = [
    ("ピースコミュニティ", "月額 1,320円", "年間 14,400円（税込）", "JACK12の理念・活動・学びに継続して触れる入口。年間払いは月額払いより1,440円お得。"),
    ("オンライン塾", "550,000円", "グループオンライン講座", "とにかく学びたい方へ。ユダヤ人に伝わる原理原則と、信頼の築き方、自分の見せ方をまとめて学べます。"),
    ("VIP塾", "3,300,000円", "マンツーマンセッション", "一緒にビジネスをしたい方へ。事業づくりやブランド、海外展開について個別に話し合い、ご縁の活かし方まで一緒に考えます。"),
]
...
def build_flyer(tone):
    ...
    # --- 3 program cards ---
    section_y = 1538
    draw.text((M, section_y), "ご縁の結び方は、3通り", font=F["subtitle"], fill=COLORS["ink"])
    ...
    note = "各プログラムは内容・参加条件を確認のうえご案内します。人脈形成、商談成立、成果を保証するものではありません。価格はすべて税込表記です。"
